...
\begin{itemize}
  \item A \emph{Continuous Range Monitoring Query} (CRMQ)
    \begin{itemize}
      \item takes an \conceptbf{indoor spatial range} $\bf \mn{R}$ as parameter
      \item keeps reporting the objects when it is registered for a certain time frame $\mn{[t_s, t_e]}$
    \end{itemize}
  \item The \conceptbf{query result} $\bf \mathcal{M}$ -- the set of moving objects in $\bf \mn{R}$ - is maintained as follows:
    \begin{equation*}
      \mn{
...
\vspace{-5pt}
\begin{equation*}
  \mn{DQHT[deviceID] = \{ (queryID, CLASS) \}}
\end{equation*}
}
...
\begin{columns}[c]
  ...
  \column{.8\textwidth}
    ...
    \begin{itemize}
      \ssize{
      \pause
      \item $\mathrm{CLASS1}$ -- \textrm{Device is fully covered in $\mn{R}$ along with cells, e.g., $\mn{(device_{16}, query_2)}$}
      \pause
      \item $\mathrm{CLASS2}$ -- \textrm{Device is fully covered but corresponding cells are not,  e.g., $\mn{(device_{13}, query_1)}$}
      \pause
      \item $\mathrm{CLASS3}$ -- \textrm{Device intersects with the query range $\mn{R}$,  e.g., $\mn{(device_{16}, query_1)}$}
      \pause
      \item $\mathrm{CLASS4}$ -- \textrm{Device is disjoint from $\mn{R}$ and at least one of its corresponding cells in $\mn{C_{ic} = \{ c|c \sqcap R \neq \varnothing \}}$,  e.g., $\mn{(device_{1}, query_1)}$}
      \pause
      \item $\mathrm{CLASS5}$ -- \textrm{Device is disjoint from $\mn{R}$ and at least one of its corresponding cells in $\mn{C_{ex} = \{ c| \{ c, c'\} \in G.E, c' \in C_{ic} \}}$, but none of them are in $\mn{C_{ic}}$, e.g., $\mn{(device_{10}, query_2)}$}
      }
    \end{itemize}
...
\ssize{
  \begin{enumerate}
    ...
    \item Lines 10--14: \textrm{Add possible devices to CriticalDeviceList $\mn{cd}$ ($\mathrm{CLASS1}$ and $\mathrm{CLASS2}$)} \pause
    ...
    \item Line 21: \textrm{Determine extended cell set $\mn{C_{ex}}$} \pause
...
\ssize{
  \begin{enumerate}
    ...
    \item Lines 32-37: \textrm{If more than one cell, check nondeterministic objects. If all its possible cells are in $\mn{C_c}$ add the object to the certain result, else uncertain result} \pause
...
\column{.7\textwidth}
  \fsize{
  Consider $\mn{query_1}$, after the object $\mn{o}$ leaves a $\mathrm{CLASS2}$ critical device $\mn{devoce_{13}}$, it should be moved from certain to uncertain result.
  \\~\\
  Let $\mn{V_{max}}$ be the maximum speed, if $\mn{R_1 = V_{max} * \Delta t}$, the certain result can be maintained without updating for an extra period of time $\mn{\Delta t}$.
  }
\end{columns}
...
\fsize{\textrm{
To analyze probability that $\mn{o}$ is in the query range $\mn{R}$. Assume that the possible locations in a given indoor space conform a uniform distribution within all reachable regions constrained by its maximum speed.
}}
\\~\\
